...
- Integration tests: **122 pass across 22 files**. Test email delivery uses an in-memory adapter and cannot contact Resend. The database still comes from the configured environment, so per-run isolated database provisioning remains required before CI.
...
- **G-08 — implemented:** administrative routes now use shared, timing-safe request authorization. Reminder cron and Payload job execution fail closed when `CRON_SECRET` is absent or invalid; jobs require an administrator when invoked through a user session; seed endpoints authenticate privileged sessions even outside production and return 404 in production; the study-plan owner route authenticates through Payload and enforces access control; and the unused public example endpoint has been removed.
...
| G-13 | Storage migration and production media delivery are incomplete   | Payload presigns direct browser uploads to Cloudflare R2, with fail-closed runtime configuration, documented CORS, broken-image fallbacks, and protected profile-reference reconciliation. Missing legacy objects still require backup restoration or replacement; deployed CORS verification, lifecycle rules, resilient multipart ingest, and CDN delivery are not implemented. | Existing media can still be stranded; large uploads remain vulnerable to connection loss; playback and downloads may still load the app or origin inefficiently. | Restore or replace reported legacy files, apply and verify R2 CORS, migrate the remaining catalogue, add lifecycle/backup policies, implement multipart ingest where needed, and deliver protected media through an entitlement-aware CDN path. |
...
| G-15 | CI and automated coverage are insufficient                       | Lint and the current integration suite now pass with coverage for access control, payments, protected content, assessment integrity, account privacy, referrals, R2 configuration, profile-media reconciliation, and selected UI behavior. CI, a per-run isolated database, current end-to-end smoke tests, and automated security/dependency/container scans remain absent.      | Regressions outside the covered services and journeys can still reach production.                                                                                | Provision isolated test infrastructure, add current end-to-end/security coverage, and require lint, types, tests, build, generated-type drift, dependency/secret scanning, and container scanning in CI before deploy.                          |
